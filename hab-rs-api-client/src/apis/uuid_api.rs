...
pub trait UuidApi: Send + Sync {
    ...
    ///
    /// 
    async fn get_uuid<>(&self, ) -> Result<String, Error<GetUuidError>>;
}

pub struct UuidApiClient {
...
#[async_trait]
impl UuidApi for UuidApiClient {
    async fn get_uuid<>(&self, ) -> Result<String, Error<GetUuidError>> {
        let local_var_configuration = &self.configuration;

        let local_var_client = &local_var_configuration.client;

        let local_var_uri_str = format!("{}/uuid", local_var_configuration.base_path);
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<GetUuidError> = serde_json::from_str(&local_var_content).ok();
