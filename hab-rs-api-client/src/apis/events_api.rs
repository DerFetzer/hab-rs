...
pub trait EventsApi: Send + Sync {
    ...
    ///
    /// 
    async fn get_events<'topics>(&self, topics: Option<&'topics str>) -> Result<(), Error<GetEventsError>>;
    ...
    ///
    /// 
    async fn update_item_list_for_state_updates<'connection_id, 'request_body>(&self, connection_id: &'connection_id str, request_body: Option<Vec<String>>) -> Result<(), Error<UpdateItemListForStateUpdatesError>>;
}
...
impl EventsApi for EventsApiClient {
    async fn get_events<'topics>(&self, topics: Option<&'topics str>) -> Result<(), Error<GetEventsError>> {
        ...
        if let Some(ref local_var_str) = topics {
            local_var_req_builder = local_var_req_builder.query(&[("topics", &local_var_str.to_string())]);
        }
    ...
    async fn init_new_state_tacker<>(&self, ) -> Result<(), Error<InitNewStateTackerError>> {
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<InitNewStateTackerError> = serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
            Err(Error::ResponseError(local_var_error))
        }
    }
    ...
    async fn update_item_list_for_state_updates<'connection_id, 'request_body>(&self, connection_id: &'connection_id str, request_body: Option<Vec<String>>) -> Result<(), Error<UpdateItemListForStateUpdatesError>> {
        ...
        let local_var_uri_str = format!("{}/events/states/{connectionId}", local_var_configuration.base_path, connectionId=crate::apis::urlencode(connection_id));
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<UpdateItemListForStateUpdatesError> = serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
            Err(Error::ResponseError(local_var_error))
        }
    }
